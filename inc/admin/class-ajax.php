...
class Ajax
{
	...
	*/
	public static function init() {
		...
		add_action( 'wp_ajax_mmps_modify_groups', array( __CLASS__, 'modify_groups' ) );
		add_action( 'wp_ajax_mmps_modify_attributes', array( __CLASS__, 'modify_attributes' ) );
		add_action( 'wp_ajax_mmps_edit_form', array( __CLASS__, 'edit_form' ) );
		add_action( 'wp_ajax_mmps_group_rearange', array( __CLASS__, 'group_re_arange' ) );
		add_action( 'wp_ajax_mmps_load_table', array( __CLASS__, 'load_table' ) );
	}
			...
			// No validation error so far
				...
				if( $check && !is_WP_Error( $check ) ) {
					...
					*/
					...
					do_action( 'mmspecs_attributes_modified', $ac_args );
