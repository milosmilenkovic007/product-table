...
if( isset( $_GET['table_id'] ) && !empty( $_GET['table_id'] ) ){
	$groups_flt     = mmspecs_get_table_groups( 'array', absint( $_GET['table_id'] ) );
		...
		<div class="mmps-box-wrapper clearfix">
			<div class="mmps-box-top clearfix">
				...
				<?php if( sizeof( $tbl_array ) > 0 ) : ?>
					...
					<select class="mm-search-by-tbl" id="search_by_table" onChange="if (this.value) window.location.href=this.value">
						<?php
						$default_url = isset( $_GET['table_id'] ) ? esc_url( remove_query_arg( 'table_id', mmspecs_current_page_url() ) ) : ''; ?>
						<option value="<?php echo $default_url; ?>"><?php _e( 'Select a table', 'product-specifications' ); ?></option>
						<?php
						foreach( $tbl_array as $table ){
							if( isset( $_GET['table_id'] ) ) {
								$url = esc_url( add_query_arg( 'table_id', $table->ID, remove_query_arg( 'table_id', mmspecs_current_page_url() ) ) );
							} else{
								$url = esc_url( add_query_arg( 'table_id', $table->ID, esc_url( mmspecs_current_page_url() ) ) );
							}
					...
					<tbody>
						...
						if( sizeof( $attributes ) > 0 ) :
							...
							<tr>
								...
								<td><h4><a href="#" data-type="attribute" data-mmpsmodal="true" class="edit" data-id="<?php echo $attr->term_id; ?>"><?php echo $attr->name; ?></a></h4></td>

								<td>
									<h4>
										<?php
										$group = get_term( get_term_meta( $attr->term_id, 'attr_group', true ), 'spec-group' );
										if (! is_wp_error($group)) echo '<a href="'. add_query_arg( 'group_id', $group->term_id, remove_query_arg('paged', mmspecs_current_page_url()) ) .'">' . $group->name . '</a>'; ?>
									</h4>
								</td>
								...
								<td class="mmps-actions">
									...
									<a href="#" role="button" class="edit" data-mmpsmodal="true" aria-label="<?php _e('Edit group', 'product-specifications'); ?>" data-type="attribute" data-id="<?php echo $attr->term_id; ?>"><i class="dashicons dashicons-welcome-write-blog"></i></a>
								</td>
			...
			<div class="mmps-buttons clearfix">
				...
				<a data-mmpsmodal="true" id="dpws_add_new_btn" class="mmps-button add-new-btn" href="#" role="button" aria-label="<?php _e('Add a new attribute', 'product-specifications'); ?>"><?php _e('Add new', 'product-specifications'); ?></a>
	...
	<form action="#" method="post" id="mmps_modify_form">
		...
		<?php if( sizeof( $tbl_array ) > 0 ) : ?>
			...
			<p>
				...
				<select name="attr_table" id="attr_table" aria-required="true" data-tables='<?php echo mmspecs_get_table_groups('json'); ?>'>
		...
		<input type="hidden" name="action" value="mmps_modify_attributes">
		<input type="hidden" name="do" value="add">
		<?php wp_nonce_field( 'mmps_modify_attributes', 'mmps_modify_attributes_nonce' ); ?>
