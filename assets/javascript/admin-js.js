...
function mmDismissAdminNotice(event, id) {
	event.preventDefault();
	var $notice = jQuery(event.target).parents('.notice');

	jQuery.ajax({
		method: 'POST',
		url: mmspecs_plugin.ajaxurl,
		data: {
			action: 'mm_dismiss_admin_notice',
			id: id
		},
...
(function( $ ){
	...
	/*!
	 * Tab Boxes by milosm
	 * @author MM - https://milos.live
	 * @version 1.2.2
	 */
	$(document).on('ready mm_dynamic', function(){
					...
					if( $(this).data("ajax-id") && $( $(this).attr("data-target") ).length === 0 ) {
						...
						$.ajax({
							...
							success : function( msg ) {
								...
								$(document).trigger('mmtabs-loaded', [true]);
							}
						});
					}
	...
	*/
	...
	/**
	 * Product tables in products edit pages
	*/
	$(function(){
		...
		$('select#spec_tables_list').on('change', function(e){
			...
			if( $(this).val() !== 0 && $(this).val() !== '0' ) {
				if( ( initial_val == 0 || initial_val == '0' ) || window.confirm('Are you sure you want to switch table? all unsaved settings will be lost.') ){
					...
					$.post( mmspecs_plugin.ajaxurl, { action: 'mmps_load_table', specs_table: $(this).val(), post_id: post_id }, function( response ){
						container.html( response );

						setTimeout(function(){
							$(document).trigger('mm_dynamic', [true]);
						}, 250);
					});
				}
				else{
		...
		$(document).on('change', '.mm-table-field-wrap .customvalue-switch', function(){
		...
		$(document).on('ready mm_dynamic', function(){
			$('.mm-table-field-wrap .customvalue-switch').change();
		});
	...
	*/
	$(function(){
		$(document).on('click', 'a[data-mmpsmodal]', function(e){
			e.preventDefault();

			var template = document.getElementById('modify_form_template').innerHTML;
			var texts    = $.parseJSON( Mustache.render( document.getElementById('mmps_texts_template').innerHTML ) );
			var elem 	 = $(this);
			...
			// create edit form modal
				...
				var data = {
					action : 'mmps_edit_form',
					id     : edit_id,
					type   : elem.data('type')
				};

				elem.css( 'opacity', '0.4' );
				$.get( mmspecs_plugin.ajaxurl, data, function(e){						
		 ...
		 * Add/edit form ajax
		*/
		$(document).on('submit', '#mmps_modify_form', function(e){
			e.preventDefault();

			var form = $(this);
			var data = $(this).serializeArray();

			$.post(mmspecs_plugin.ajaxurl, data, function(response) {
				...
				// Success
					...
					// Add handler
					$('#mmps_table_wrap').load( window.location.href + ' #mmps_table' );
		...
		*/
		$(function(){
			$(document).on('change', '.mmps-table .check-column input', function(){
				var table = $(this).parents('.mmps-table');
		...
		*/
		$(document).on('click', '#mmps_table .delete, #dpws_bulk_delete_btn', function(e){
			e.preventDefault();
			var id = $(this).data('id');
			var template = $.parseJSON(  Mustache.render( document.getElementById('mmps_delete_template').innerHTML ) );
			...
			if( !$(this).is('[disabled]') ){
				...
				if (confirm) {
					var action = template.data.type == 'attribute' ? 'mmps_modify_attributes' : 'mmps_modify_groups';

					$.post(mmspecs_plugin.ajaxurl, {action : action, do: 'delete', id: id }, function(response) {
		 ...
		 * Re-arrange handler
		*/
		$(document).on('click', '#mmps_table .re-arange', function(e){
			e.preventDefault();
			var texts = $.parseJSON( Mustache.render( document.getElementById('mmps_texts_template').innerHTML ) );

			$.get( mmspecs_plugin.ajaxurl, { action: 'mmps_group_rearange', id: $(this).data('id') }, function( response ){
		...
		*/
		$(function(){
			...
			$(document).on('change', '.mmsp-meta-item input[type=checkbox]', function(e){
				var val = $(this).val();
	...
	$('#mmps_import_data_form').submit(function(e) {
		e.preventDefault();

		var data = new FormData(this);

		$('#mmspecs_import_results').html('<div class="notice"><p>'+ mmspecs_plugin.i18n.importing_message + '</p></div>');	
		...
		$.ajax({
			...
			success: function(res) {
				if (res.success) {
					$('#mmspecs_import_results').html('<div class="updated success"><p>'+ res.data.message + '</p></div>')
					
				} else {
					$('#mmspecs_import_results').html('<div class="error"><p>'+ res.data.message + '</p></div>')

				}
			},
			error: function() {
				$('#mmspecs_import_results').html('<div class="error"><p>'+ mmspecs_plugin.i18n.unknown_error + '</p></div>')
			}
		})
	})
