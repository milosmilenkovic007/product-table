...
class Specification_Tables
{
	...
	**/
	public static function meta_box(){
		add_meta_box( 'mm-specs-table-metas', __( 'Table Options', 'product-specifications' ), array( __CLASS__, 'content_spec_table' ), 'specs-table', 'normal', 'high' ); // Table settings in table post type

		add_meta_box( 'mmps-specs-table', __( 'Product table', 'product-specifications' ), array( __CLASS__, 'content_product_specs' ), array( 'product' ), 'normal', 'high' ); // Table input meta boxes
	}
	...
	**/
	public static function save_table_metabox( $post_id ){
		...
		if( !isset( $_POST['mmps_metabox_nonce'] ) || !wp_verify_nonce( $_POST['mmps_metabox_nonce'], 'mm-specs-table-metas' ) ) return;
	...
	**/
	public static function save_product_table( $post_id ){
		...
		update_post_meta($post_id, '_mmps_table', $table_id);

		if (!$table_id) {
			return;
		}

		$table_array = array_filter( $_POST['mm-attr'] );
		$table_result = array();
		...
		foreach( $table_array as $group_id => $group_attributes ){
			...
			foreach( $group_attributes as $attr_id => $value ){
				if( empty( $value ) || 'mmspecs_chk_none' == $value ) continue;
		...
		if( !get_post_meta( $post_id, '_mmps_specification_table', true ) || get_post_meta( $post_id, '_mmps_specification_table', true ) == '' ){
			delete_post_meta( $post_id, '_mmps_specification_table' );
			add_post_meta( $post_id, '_mmps_specification_table', $table_result );
		} else{
			if( get_post_meta( $post_id, '_mmps_specification_table', true ) != $table_result )
				update_post_meta( $post_id, '_mmps_specification_table', $table_result );
		}
	}
