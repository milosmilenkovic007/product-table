...
class Import_Export
{
	...
	public static function init() {
		add_action('wp_ajax_mmspecs_export_data', [__CLASS__, 'download']);
		add_action('wp_ajax_mmspecs_import_data', [__CLASS__, 'import_cb']);
	}
	...
	public static function import_cb() {
		...
		if (false !== get_transient('mmspecs_data_migrating')) {
			wp_send_json_error([
		...
		set_transient('mmspecs_data_migrating', true, MINUTE_IN_SECONDS * 30);
	...
	public static function download() {
		if (! wp_verify_nonce($_POST['mms_ex_nonce'], 'mmspecs_nonce_export')) {
	...
	public static function export_filename() {
		return 'mm-specs-table-' . date('j-m-Y') . '-' . rand(1, 1e6) . '.json';
	}

	public static function export($format = 'array', $include_products = true) {
		$results = [];
		$i = 0;
		foreach (mmspecs_get_table_groups() as $table) {
			$results[$i] = [
			...
			foreach ($table['groups'] as $group) {
				...
				foreach ((mmspecs_get_attributes_by_group($group['term_id']) ?: []) as $attr) {
					$attributes[] = [
						'title' 	=> $attr->name,
						'slug' 		=> $attr->slug,
						'id' 		=> $attr->term_id,
			...
			if ($include_products) {
				...
				foreach ($products_list->posts as $product) {
					$results[$i]['products'][] = [
						...
						'table' => mmspecs_get_table_result($product->ID)
					];
				}
			}
		...
		foreach ($data as $table) {
			...
			foreach ($table['skleton'] as $group) {
				...
				if ($new_group_id && ! is_wp_error($new_group_id)) {
					$new_order = array_map(function($el) use($ids_map) {
						if (isset($ids_map[$el])) {
			...
			$table_g_order = array_map(function($el) use($ids_map) {
				if (isset($ids_map[$el])) {
					return str_replace($el, $ids_map[$el], $el);
			...
			if (isset($table['products'])) {
				foreach ($table['products'] as $product) {
					...
					// Get product by slug
					$import_post_field = apply_filters('mmspecs_import_post_field_key', 'post_name');
					...
					if ($product_id) {
						update_post_meta($product_id, '_mmps_table', $new_table_id);
						update_post_meta($product_id, '_mmps_specification_table', $product['table']);
					}
				}
			}
		}
