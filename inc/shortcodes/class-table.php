...
namespace MMSpecificationsTable\Shortcodes;
...
class Table {
	...
	public static function shortcode_handler( $atts, $content = null ) {
		...
		$table = mmspecs_get_table_result( $post_id );
		$args = array( 'post_id' => $post_id, 'table' => $table );

		ob_start();
		mmspecs_table_template_part('shortcodes/table', $args);
		$output = apply_filters('mm_specs_table_shortcode_output', ob_get_clean(), $args);
