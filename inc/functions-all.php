 ...
 * @return string
*/
if(! function_exists('mmspecs_dumpit')) {
	function mmspecs_dumpit( $val ) {
 ...
 * @return string
*/
if( !function_exists('mmspecs_current_page_url') ) {
	function mmspecs_current_page_url() {
		$pageURL = 'http';
 ...
 * @return string
*/
if( !function_exists('mmspecs_strip_some') ) {
	function mmspecs_strip_some( $string, $tags = array() ) {
		foreach ($tags as $tag) {
 ...
 * @return Array
 */
if( !function_exists('mmspecs_get_attributes_by_group') ) {
	function mmspecs_get_attributes_by_group( $group_id = false ){
		if( !$group_id ) return;
 ...
 * @return string
 */
if( ! function_exists('mmspecs_encodeURIComponent') ) {
	function mmspecs_encodeURIComponent($str) {
 ...
 * @return mixed
*/
if( !function_exists('mmspecs_attr_value_by') ){
	function mmspecs_attr_value_by( $post_id = '', $field, $value ) {
		if( !$post_id ){
	        global $post;
	        $post_id = $post->ID;
	    }
	    ...
	    $table = mmspecs_get_table_result( $post_id ); // The large array
...
if( ! function_exists('mmspecs_product_has_specs_table') ){
	function mmspecs_product_has_specs_table( $post_id = '' ){
		if( ! $post_id ){
			global $post;
			$post_id = $post->ID;
		}

		$table_id = get_post_meta($post_id, '_mmps_table', true);
		$table = get_post_meta($post_id, '_mmps_specification_table', true);

		return !empty($table_id) && !empty($table);
	}
}

if( ! function_exists('mmspecs_spec_group_has_duplicates') ){
	function mmspecs_spec_group_has_duplicates( $name, $tax = 'spec-group' ){
 ...
 */
if (! function_exists('mmspecs_table_template_part')) {
    function mmspecs_table_template_part($path = '', $args = []) {
        $theme_path = trailingslashit(get_stylesheet_directory()) . 'specs-table/' . $path . '.php';
        $plugin_path = trailingslashit(MMSPECS_ABSPATH) . 'templates/' . $path . '.php';
 ...
 */
if( !function_exists('mmspecs_get_table_result') ){
	function mmspecs_get_table_result( $post_id = '', $output = 'array', $hide_empty = true ) {
		if( !$post_id ){
			global $post;
			$post_id = $post->ID;
		}
 ...
 * @return mixed
*/
if( !function_exists('mmspecs_get_table_groups') ){
	function mmspecs_get_table_groups( $format = 'array', $table_id = false ){
		$output = array();
